(() => {
  ...
  const WEBSITE_JOIN='/join/?src=website';
  ...
  function canonicalizeDiscordLinks(root=document){
    const links=[];
    if(root?.matches?.('a[href]'))links.push(root);
    if(root?.querySelectorAll)links.push(...root.querySelectorAll('a[href]'));
    links.forEach(link=>{
      const href=link.getAttribute('href')||'';
      if(!/^https:\/\/(?:www\.)?(?:discord\.gg|discord\.com\/invite)\//i.test(href))return;
      link.setAttribute('href',WEBSITE_JOIN);
      link.removeAttribute('target');
      const rel=(link.getAttribute('rel')||'').split(/\s+/).filter(Boolean).filter(value=>!['noopener','noreferrer'].includes(value));
      if(rel.length)link.setAttribute('rel',rel.join(' '));else link.removeAttribute('rel');
      link.dataset.joinSource='website';
    });
  }

  function watchDiscordLinks(){
    canonicalizeDiscordLinks();
    if(!document.body||typeof MutationObserver==='undefined')return;
    const observer=new MutationObserver(records=>{
      records.forEach(record=>record.addedNodes.forEach(node=>{
        if(node.nodeType===1)canonicalizeDiscordLinks(node);
      }));
    });
  ...
  async function start(){watchDiscordLinks();if(document.body.dataset.page!=='home')return;ensureHeroVideo();try{const [league,profile]=await Promise.all([json('/data/league.json'),json('/data/player-card-hitman.json')]);mountPlayerWorld(league,profile);}catch{}}
